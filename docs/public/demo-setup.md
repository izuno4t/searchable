...
Docker Compose を用いた最小デモ環境の構築方法。
...
初回はビルドに数分かかる。`Started SearchableUiApplication` が表示
されたら準備完了。
...
`docker/demo-data/` 配下の Markdown 4ファイルが `demo` Namespace に
登録される。
...
| `SEARCHABLE_GLOBAL_DEFAULT_ARCHITECTURE` | `HYBRID` | 既定検索アーキテクチャ |

任意の Spring Boot プロパティを `SEARCHABLE_XXX_YYY` 形式の環境変数
として渡すことで上書き可能（kebab-case → SCREAMING_SNAKE_CASE）。

## 7. ボリューム

`searchable-data` 名のローカルボリュームに永続化される。
...
- Maven 依存を一度ローカル(`~/.m2`) でビルドしてから docker
  build すると速い
- 初回は Lucene / Spring Boot 等で 300MB 程度ダウンロードする
...
- サーバー起動直後だと初期化中の場合あり。`seed.sh` はリトライ
  ループを持っているが、それでも失敗する場合は再実行
