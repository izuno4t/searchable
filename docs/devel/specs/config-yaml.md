...
デシリアライズして組み立てる `ApplicationConfig` の構造を正本として定義する。
...
`ApplicationConfig`
(`searchable-core/src/main/java/io/searchable/core/application/config/ApplicationConfig.java:21-27`)
に対応する。

| フィールド | 型 | 必須 | デフォルト | 説明 |
| --- | --- | --- | --- | --- |
| `data-directory` | path (string) | 必須 | なし | データ全体の anchor となるディレクトリ。本フィールドが `null` の YAML は `IllegalStateException` で拒否される (`ApplicationConfig.java:30`)。 |
| `persistence` | object | 必須 | なし | メタデータ DB 接続設定。`null` の YAML は拒否される (`ApplicationConfig.java:31`)。詳細は §3。 |
...
| `global` | object | 任意 | `GlobalConfig.defaults()` | namespace 既定値 (検索アーキテクチャー・戦略・順序・アナライザー)。詳細は §6。 |
...
`ApplicationConfig.normalize` によって以下のように書き換えられる
(`ApplicationConfig.java:96-178`、ADR-0002)。

- 対象: `jdbc:h2:` から始まり、かつ次の prefix を含まない URL
  - 非対象 prefix: `mem:` / `tcp:` / `ssl:` / `zip:` / `nio:` / `nioMapped:` /
    `nioMemFS:` / `nioMemLZF:` / `memFS:` / `memLZF:` (`ApplicationConfig.java:111-115`)
- 書き換え内容
  1. 相対パスを `data-directory` (絶対化済み) を基準に絶対パスへ正規化
  2. クエリパラメーターに `AUTO_SERVER=TRUE` が含まれていなければ末尾に付加
     (`ApplicationConfig.java:174-176`)
- `~` で始まるパスは H2 のチルダ展開に委ね、パス自体は触らないが
  `AUTO_SERVER=TRUE` の付加は行う (`ApplicationConfig.java:166-167`)
- 非対象モード (`mem:` / `tcp:` 等) の URL、および非 H2 URL (`jdbc:postgresql:` 等)
  は変更されない (`ApplicationConfig.java:138-145`)
...
- セクション自体が未指定 → `IndexConfig.defaults()` が `Path.of("./data/indexes")` を持つ sentinel として適用される (`IndexConfig.java:29-31`、`ApplicationConfig.java:32`)
- `ConfigLoader.load(Path)` 経由では `ApplicationConfig.normalize` が以下の規則で書き換える (`ApplicationConfig.java:79-87`)
...
- セクション自体が未指定 → `PluginsConfig.classpathOnly()` (`directory = null`)
  が適用される (`PluginsConfig.java:12-14`、`ApplicationConfig.java:33`)
- `directory` が指定されている場合、`ApplicationConfig.normalize` が `index.directory`
  と同じルールで `data-directory` 基準に絶対化する (`ApplicationConfig.java:89-94`)。
  `null` の場合は `null` のまま保持される。

## 6. `global` セクション

`GlobalConfig`
(`searchable-core/src/main/java/io/searchable/core/application/config/GlobalConfig.java:18-48`)
に対応する。
namespace を新規作成する際に値が未指定の場合のフォールバックとして使われる。

| フィールド | 型 | 必須 | デフォルト | 説明 |
| --- | --- | --- | --- | --- |
| `default-architecture` | enum | 必須 (セクションを書く場合) | `FULL_TEXT` (セクション省略時) | `SearchType` の値。`null` は `NullPointerException` を投げる (`GlobalConfig.java:26`)。 |
| `default-search-strategy` | enum | 必須 (セクションを書く場合) | `SEQUENTIAL` (セクション省略時) | `SearchStrategy` の値。`null` は拒否される (`GlobalConfig.java:27`)。 |
| `default-search-order` | enum | 必須 (セクションを書く場合) | `FULL_TEXT_FIRST` (セクション省略時) | `SearchOrder` の値。`null` は拒否される (`GlobalConfig.java:28`)。 |
| `analyzer` | enum | 任意 | `KUROMOJI` | 日本語アナライザー実装。`null` は `KUROMOJI` にフォールバック (`GlobalConfig.java:29`)。 |

セクション自体を省略すると `GlobalConfig.defaults()` (全フィールドがデフォルト) が適用される
(`GlobalConfig.java:40-47`、`ApplicationConfig.java:34`)。
...
`ApplicationConfig.normalize(raw, base)` を呼び出してすべてのパス系フィールドを絶対化する
(`ConfigLoader.java:32-44`)。

| フィールド | base | 解決規則 |
| --- | --- | --- |
| `data-directory` | YAML ファイルの親ディレクトリ (絶対化済み) | 絶対なら `normalize()` のみ。相対は base に解決 (`ApplicationConfig.java:60-77`)。 |
| `index.directory` | 正規化後の `data-directory` | sentinel (`./data/indexes`) なら `<data-directory>/indexes` に置換。絶対なら as-is。相対は base に解決 (`ApplicationConfig.java:79-87`)。 |
| `plugins.directory` | 正規化後の `data-directory` | `null` はそのまま。絶対なら as-is。相対は base に解決 (`ApplicationConfig.java:89-94`)。 |
...
| `data-directory` が未指定 | `IllegalStateException` (cause: `NullPointerException`) | `ApplicationConfig.java:30` 由来 → `ConfigLoader.java:51` でラップ | `dataDirectory must not be null` |
| `persistence` セクションが未指定 | `IllegalStateException` (cause: `NullPointerException`) | `ApplicationConfig.java:31` 由来 | `persistence must not be null` |
...
| `global.default-architecture` / `default-search-strategy` / `default-search-order` が `null` (セクション自体は記述あり) | `IllegalStateException` (cause: `NullPointerException`) | `GlobalConfig.java:26-28` 由来 | `defaultArchitecture must not be null` 等 |
...
`GlobalConfigProvider` (`searchable-core/src/main/java/io/searchable/core/application/config/GlobalConfigProvider.java:13-29`)
は `GlobalConfig` を保持する `AtomicReference` ベースのホルダーであり、admin UI が
`update(GlobalConfig)` を呼ぶことでアプリケーション再起動なしに `global` セクション相当の
