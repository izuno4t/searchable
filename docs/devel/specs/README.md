...
Javadoc サイトの生成・公開有無は別途決定する (本リポジトリの maven 設定に `maven-javadoc-plugin` の active な設定がないため、現状は IDE / ソース直接参照前提)。
...
| [`document-metadata.md`](document-metadata.md) | ドキュメント取込時の予約メタデータキー (url・contentType・category・lang・tags ほか) とパーサ別自動付与挙動 |
...
- REST API: [`examples/api/api-specification.ja.md`](../../../examples/api/api-specification.ja.md), [`examples/api/openapi.yaml`](../../../examples/api/openapi.yaml)
