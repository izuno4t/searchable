...
| オプション | 型 | 必須 | 既定値 | 説明 |
| --- | --- | --- | --- | --- |
| `-c`, `--config <path>` | `java.nio.file.Path` | サブコマンド側で利用するため実質必須 | なし | YAML 形式の `SearchableConfig` ファイルパス |
...
- `CliRuntime.loadConfig(configPath)` で `SearchableConfig` を読み込むのみで、`SearchableLibrary` は
...
各値は `SearchableConfig` のサブ設定 record から取得した文字列表現
