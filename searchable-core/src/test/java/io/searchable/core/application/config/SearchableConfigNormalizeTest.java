...
/**
 * Resolution rules for {@link SearchableConfig#normalize(SearchableConfig, Path)};
 * see {@code docs/devel/adr/0002-data-directory-relative-path-resolution.md}.
 */
class SearchableConfigNormalizeTest {

    @TempDir Path tempDir;

    private SearchableConfig raw(final Path dataDir, final PersistenceConfig p,
                                  final IndexConfig idx, final PluginsConfig pl) {
        return new SearchableConfig(dataDir, p, idx, pl, SearchableGlobalConfig.defaults());
    }

    @Test
    void absoluteDataDirectoryStaysAsIs() {
        final Path abs = tempDir.resolve("absolute-data").toAbsolutePath();
        final SearchableConfig in = raw(abs,
            new PersistenceConfig("H2", "jdbc:h2:mem:test", "sa", ""),
            new IndexConfig(abs.resolve("idx")),
            PluginsConfig.classpathOnly());

        final SearchableConfig out = SearchableConfig.normalize(in, tempDir);
    ...
    void relativeDataDirectoryResolvesAgainstBase() {
        final SearchableConfig in = raw(Path.of("./mydata"),
            new PersistenceConfig("H2", "jdbc:h2:mem:test", "sa", ""),
    ...
    void unsetIndexDirectoryDefaultsToDataDirectoryIndexes() {
        // IndexConfig.defaults() is what the SearchableConfig record constructor
        // installs when YAML omits the `index` key. Normalize must replace that
        // sentinel with <data-directory>/indexes.
        final SearchableConfig in = raw(Path.of("./data"),
            new PersistenceConfig("H2", "jdbc:h2:mem:test", "sa", ""),
